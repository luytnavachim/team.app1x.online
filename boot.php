...
function syncKitPrintFromPhotos(mysqli $db): void {
    ...
    $rows = [
        ...
        13 => ['print_rohda' => 1, 'print_initials' => 1, 'print_sponsor' => 0, 'print_sponsor_back' => 0, 'print_sponsor_padded' => 0, 'print_sponsor_jacket' => 1, 'print_sponsor_bag' => 0, 'print_name_back' => 0, 'print_staff_text' => 0],
...
function ensurePackageTypes(mysqli $db): void {
    ...
    foreach ($rows as $id => $r) {
        ...
        if ($found) {
            ...
            if ($id === 15) {
                $fix = $db->prepare('UPDATE clothing_types SET name=?, display_name=?, article_number=?, description=?, updated_at=NOW() WHERE id=?');
                $fix->bind_param('ssssi', $name, $display, $article, $desc, $fid);
                $fix->execute();
            }
            continue;
        }
